...
#E. Check relationship between shoe size and height
```{r}
...
```
8.
#A. Create data frame for student grades
```{r}
...
```{r}
for(i in 1:nrow(grades_data)) {
  max_score <- max(grades_data$Grade1[i], grades_data$Grade2[i], grades_data$Grade3[i], grades_data$Grade4[i])
  if(max_score > 90) {
    print(paste(grades_data$Name[i], "'s highest grade this semester is", max_score))
  }
}
```